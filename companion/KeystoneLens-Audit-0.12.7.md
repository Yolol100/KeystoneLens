...
| Windows companion | 2 | 2 | 0 | 99 tests, compileall, Linux/Xvfb UI-smoke, failure injection |
| Installer | 1 | 1 | 0 | source failure model, Go vet/build/PE resources; native Windows niet uitgevoerd |
...
Alle vier staan **CONFIRMED → FIXED**. Er zijn geen open CONFIRMED defects in de uitvoerbare scope.
...
- Setup/launcher/uninstaller cross-build als Windows x64 GUI PE en resourceverificatie slagen; artifact blijft unsigned.
...
- `OpenProcess`-gebruik is begrensd tot het eigen child-process Job Object en exact-path uninstall process matching; geen gameprocess targeting gevonden.
...
- Native Windows taskbar, first-start, SmartScreen en echte error-dialog UX zijn niet uitgevoerd.

## Verification

- Python unit/regression tests: **PASS — 99/99**.
...
| 17–18 | installer/elevation | SOURCE/BUILD DONE; NATIVE INSTALL/ELEVATION SKIPPED |
