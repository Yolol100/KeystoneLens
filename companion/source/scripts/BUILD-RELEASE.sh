...
for base in [
    ...
    for path in base.rglob('*'):
        ...
        if '/build/' in '/' + rel or '/release/' in '/' + rel:
            continue
...
with zipfile.ZipFile(out, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
    for path in selected:
        ...
        info.compress_type = zipfile.ZIP_DEFLATED
        info.create_system = 3
        mode = 0o755 if path.suffix in {'.sh', '.py'} or path.name in {'START-COMPANION.cmd'} else 0o644
...
(
  ...
  pytest -q >/dev/null
  python3 -m compileall -q app/keystonelens_companion
  grep -Fx "$VERSION" VERSION >/dev/null
  grep -Fx "__version__ = \"$VERSION\"" app/keystonelens_companion/__init__.py >/dev/null
  grep -Fx "## Version: $VERSION" addon/KeystoneLensBridge/KeystoneLensBridge.toc >/dev/null
  test -f portable/START-COMPANION.cmd
  test -f portable/build-portable.ps1
  test -f portable/portable_launcher.py
  test -f runtime/windows/python-runtime.json
  test -f runtime/windows/requirements-runtime.lock
  test ! -e installer
)
