...
func retailWoWPIDs() map[uint32]struct{} {
	...
	for ok != 0 {
		...
		if strings.EqualFold(name, "Wow.exe") {
			procRaw, _, _ := openProcess.Call(processQueryLimitedInformation, 0, uintptr(entry.ProcessID))
			if procRaw != 0 {
				...
				syscall.CloseHandle(syscall.Handle(procRaw))
				if queried != 0 && isRetailWoWPath(syscall.UTF16ToString(buf[:size])) {
